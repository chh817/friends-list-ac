...
<div class="card m-3">
  ...
  Info
    ...
    <button class="btn btn-primary btn-add" data-id="${datum.id}">+</button>
  </div>
</div>
`
  })
...
dataPanel.addEventListener("click", function dataPanelRsp(e) {
  ...
  if (target.matches(".btn-modal-info")) {
    ...
    addFavorites(Number(target.dataset.id))
  }
})